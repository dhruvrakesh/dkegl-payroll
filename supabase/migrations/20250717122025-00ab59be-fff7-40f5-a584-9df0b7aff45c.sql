-- Panchkula Unit Salary & Leave Management Implementation

-- 1. Update payroll settings for correct deduction rates
UPDATE payroll_settings 
SET 
  esi_rate = 0.75,  -- Correct ESI rate to 0.75%
  epf_rate = 12.0,  -- Keep EPF at 12%
  labour_welfare_fund = 31,  -- Fixed LWF amount
  updated_at = now()
...
BEGIN
  ...
  epf_deduction := basic_earned * (v_settings.epf_rate / 100.0);
  esi_deduction := gross_salary * (v_settings.esi_rate / 100.0);
  lwf_deduction := v_settings.labour_welfare_fund;
...
BEGIN
  ...
  LOOP
    ...
    IF DATE_TRUNC('month', emp_record.joining_date) <= MAKE_DATE(p_year, p_month, 1) THEN
      ...
      VALUES (emp_record.id, p_year, v_settings.monthly_cl_accrual, v_settings.el_accrual_ratio)
      ON CONFLICT (employee_id, year) 
      DO UPDATE SET 
        casual_leave_balance = employee_leave_balances.casual_leave_balance + v_settings.monthly_cl_accrual,
        earned_leave_balance = employee_leave_balances.earned_leave_balance + v_settings.el_accrual_ratio,
        updated_at = now();
