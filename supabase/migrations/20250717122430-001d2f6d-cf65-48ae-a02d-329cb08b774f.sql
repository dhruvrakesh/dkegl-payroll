-- Panchkula Unit Salary & Leave Management Implementation (Final)

-- 1. Update payroll settings for correct deduction rates
UPDATE payroll_settings 
SET 
  esi_rate = 0.75,  -- Correct ESI rate to 0.75%
  pf_rate = 12.0,   -- Keep EPF at 12%
  lwf_amount = 31,  -- Fixed LWF amount
  updated_at = now()
WHERE setting_id IS NOT NULL;

-- 2. Add monthly leave accrual settings to payroll_settings
ALTER TABLE payroll_settings 
ADD COLUMN IF NOT EXISTS monthly_cl_accrual INTEGER DEFAULT 1,
ADD COLUMN IF NOT EXISTS el_accrual_ratio NUMERIC DEFAULT 1.67, -- 1 day per 20 working days
...
-- 4. Enhanced 30-day based salary calculation function for Panchkula
CREATE OR REPLACE FUNCTION calculate_panchkula_salary(
  p_employee_id UUID,
  p_month DATE,
  p_basic_salary NUMERIC,
  p_hra_amount NUMERIC,
...
BEGIN
  ...
  -- Calculate earnings based on 30-day uniform base (Panchkula method)
  basic_earned := ROUND((p_basic_salary / 30.0) * v_paid_days, 2);
  hra_earned := ROUND((p_hra_amount / 30.0) * v_paid_days, 2);
  other_earned := ROUND((p_other_allowances / 30.0) * v_paid_days, 2);
  gross_salary := basic_earned + hra_earned + other_earned;
  
  -- Calculate deductions using Panchkula rates
  epf_deduction := ROUND(basic_earned * (v_settings.pf_rate / 100.0), 2);  -- EPF on basic only
  esi_deduction := ROUND(gross_salary * (v_settings.esi_rate / 100.0), 2); -- ESI on gross salary
  lwf_deduction := v_settings.lwf_amount; -- Fixed amount
...
BEGIN
  ...
  LOOP
    ...
    IF DATE_TRUNC('month', emp_record.joining_date) <= MAKE_DATE(p_year, p_month, 1) THEN
      -- Accrue Casual Leave (1 per month) and Earned Leave
...
BEGIN
  ...
  IF v_is_sunday THEN
    ...
    ELSIF NEW.status NOT IN ('PRESENT', 'WEEKLY_OFF') THEN
      -- Sundays should generally be WEEKLY_OFF (paid weekly off)
  ...
  -- Validate leave consumption for CL and EL
  ...
  -- Prevent PRESENT status with 0 hours worked
...
-- Replace the existing trigger with enhanced Panchkula validation
DROP TRIGGER IF EXISTS attendance_validation_trigger ON attendance;
DROP TRIGGER IF EXISTS panchkula_attendance_validation_trigger ON attendance;
CREATE TRIGGER panchkula_attendance_validation_trigger
  BEFORE INSERT OR UPDATE ON attendance
  FOR EACH ROW EXECUTE FUNCTION validate_panchkula_attendance();

-- 8. Initialize leave balances for all existing employees
INSERT INTO employee_leave_balances (employee_id, year, casual_leave_balance, earned_leave_balance)
SELECT 
  id,
  EXTRACT(YEAR FROM CURRENT_DATE),
  12, -- Annual CL allocation (1 per month * 12)
  20  -- Annual EL allocation (as per Labour Act)
FROM payroll_employees 
WHERE active = true
ON CONFLICT (employee_id, year) DO NOTHING;

-- 9. Create enhanced payroll calculation view
